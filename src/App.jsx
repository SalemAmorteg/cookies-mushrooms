...
// ── datos (fuente: maestro_de_citas.md) ──────────────────────────────────────

// const INIT_CITAS = [
  // { id:"001", fechaRaw:"2026-04-11", plan:"Nuestra primera cita", categoria:"Cultura",
     //highlight:"La despedida de 40 minutos 🚪", sentimiento:"Mil emociones", emoji:"🌪️", tipo:"recuerdo" },
   //{ id:"002", fechaRaw:"2026-04-25", plan:"Salem's Birthday", categoria:"Fecha Especial",
    // highlight:"When we hugged after not seeing each other in 15 days. The polaroids. Singing in the car coming back to Bogotá",
    // sentimiento:"Expectancy - joy - panic", emoji:"🤯✨", tipo:"recuerdo" },
  // { id:"003", fechaRaw:"2026-05-09", plan:"Crepes & Waffles/Aribnb", categoria:"Fecha Especial",
   //  highlight:"¿Tú quieres ser mi novia?", sentimiento:"AMOR", emoji:"❤️", tipo:"recuerdo" },
 //  { id:"004", fechaRaw:"2026-05-10", plan:"Día de la Madre", categoria:"Fecha Especial",
  //  highlight:"Conociendo al Sr. Armando y la Sra. Olga", sentimiento:"Nervios al 1000%", emoji:"😬", tipo:"recuerdo" },
//];
//const INIT_PLANES = [];
...
function EditModal({ item, onSave, onClose }) {
  ...
  const [f, setF] = useState({ fechaRaw:item.fechaRaw, plan:item.plan, categoria:item.categoria,
    highlight:item.highlight, sentimiento:item.sentimiento, emoji:item.emoji });
  const [saved, setSaved] = useState(false);
  const ac = fut ? "#fff" : "#FDE047";
  const inp = { background:"#000", border:"2px solid #333", color:"#fff", padding:"10px 12px",
    fontSize:12, width:"100%", fontFamily:"'Space Mono',monospace", outline:"none" };
  function ch(e) { setF(p => ({ ...p, [e.target.name]: e.target.value })); }
  function doSave() { onSave({ ...item, ...f }); setSaved(true); }
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", zIndex:999,
      display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ background:"#111", border:`4px solid ${ac}`, padding:28,
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
          ...
          <button onClick={onClose} style={{ background:"#222", border:"2px solid #444", color:"#fff", width:32, height:32,
            display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
            <X size={16}/>
          </button>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
          {[["📅 Fecha","fechaRaw","date"],["🗂 Categoría","categoria","select"]].map(([lbl,name,type]) => (
            <div key={name} style={{ display:"flex", flexDirection:"column", gap:6 }}>
              <label style={{ fontSize:10, fontWeight:700, letterSpacing:2, textTransform:"uppercase", color:ac }}>{lbl}</label>
              {type === "select"
                ? <select name={name} value={f[name]} onChange={ch} style={inp}>
                    {CATS.slice(1).map(c => <option key={c}>{c}</option>)}
                  </select>
                : <input type={type} name={name} value={f[name]} onChange={ch} style={{...inp, colorScheme:"dark"}}/>}
            </div>
          ))}
          ...
          {[["💛 Sentimiento","sentimiento"],["✨ Emoji","emoji"]].map(([lbl,name]) => (
            <div key={name} style={{ display:"flex", flexDirection:"column", gap:6 }}>
              <label style={{ fontSize:10, fontWeight:700, letterSpacing:2, textTransform:"uppercase", color:ac }}>{lbl}</label>
              <input name={name} value={f[name]} onChange={ch} style={inp}/>
            </div>
          ))}
        </div>
        <div style={{ display:"flex", gap:10, marginTop:16 }}>
          <button onClick={onClose} style={{ flex:1, background:"#000", border:"3px solid #444", color:"#666",
            padding:"10px", fontFamily:"'Bebas Neue',sans-serif", fontSize:16, letterSpacing:2, cursor:"pointer" }}>
            {saved ? "CERRAR" : "CANCELAR"}
          </button>
          {!saved && (
            <button onClick={doSave} style={{ flex:2, background:ac, border:`3px solid ${ac}`, color:"#000",
        ...
        {!done ? (
          <>
            ...
            <div style={{ display:"flex", gap:10 }}>
              ...
              <button onClick={go} style={{ flex:1, background:"#ff3333", border:"3px solid #ff3333", color:"#fff",
                padding:"10px", fontFamily:"'Bebas Neue',sans-serif", fontSize:16, letterSpacing:2, cursor:"pointer" }}>
                SÍ, ELIMINAR
              </button>
            </div>
          ...
          <>
            ...
            <button onClick={onClose} style={{ width:"100%", marginTop:16, background:"#FDE047", border:"3px solid #000",
...
function CardActions({ onEdit, onDelete, dark }) {
  return (
    <div style={{ display:"flex", gap:6, marginTop:10 }}>
      <button onClick={onEdit} style={{ flex:1, background:"#000", border:`2px solid ${dark?"#333":"#00000033"}`,
        color:dark?"#FDE047":"#000", padding:"6px 0",
        fontFamily:"'Bebas Neue',sans-serif", fontSize:13, letterSpacing:1, cursor:"pointer" }}>
        ✏️ EDITAR
      </button>
      <button onClick={onDelete} style={{ background:"#000", border:"2px solid #ff333366",
...
async function submit() {
  ...
  function saveEdit(u) {
    if (u.tipo === "recuerdo") setCitas(p => p.map(c => c.id === u.id ? u : c));
    else setPlanes(p => p.map(c => c.id === u.id ? u : c));
    setEditItem(null);
  }
  ...
  const SI = { background:"#000", border:"2px solid #333", color:"#fff", padding:"10px 12px",
        ...
        <div style={{ maxWidth:920, margin:"0 auto", display:"flex", minWidth:"max-content" }}>
          {TABS.map(([k,l]) => (
            <button key={k} onClick={() => setTab(k)} className={tab===k?"ta":""}
              style={{ padding:"12px 18px", fontFamily:"'Space Mono'", fontSize:10, fontWeight:700,
                textTransform:"uppercase", letterSpacing:2, color:tab===k?"#FDE047":"#555",
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:24 }}>
              {CATS.map(cat => (
                <button key={cat} onClick={() => setFiltro(cat)} className={filtro===cat?"fa":""}
                  style={{ background:"transparent", border:"2px solid #333", color:"#555", padding:"6px 12px",
        ...
        {tab === "proximos" && (
          <>
            {prox && (() => {
              ...
              return (
                <div style={{ background:"#FDE047", border:"4px solid #000", padding:20, marginBottom:24,
                    ...
                    <div key={p.id} className="ch" style={{ background:"#fff", border:"4px solid #000", position:"relative", overflow:"hidden" }}>
                      ...
                      <div style={{ padding:"14px 12px" }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:4 }}>
                          <div style={{ background:esHoy?"#FDE047":esPas?"#222":"#0a0a0a",
        ...
        {tab === "nueva" && (
          <div style={{ background:"#111", border:`4px solid ${tipoForm==="futuro"?"#fff":"#FDE047"}`, padding:24, transition:"border-color .2s" }}>
            ...
            <div style={{ fontSize:10, color:"#555", letterSpacing:2, textTransform:"uppercase", marginBottom:20 }}>
              La fila Markdown aparece automáticamente al guardar
            </div>
            ...
            <div style={{ display:"flex", background:"#000", border:"3px solid #333", overflow:"hidden", marginBottom:20 }}>
              <button onClick={() => { setTipoForm("recuerdo"); setLastRow(null); }}
                style={{ flex:1, padding:"10px 8px", fontFamily:"'Bebas Neue'", fontSize:16, letterSpacing:2, border:"none",
            ...
            <button onClick={submit}
              style={{ background:tipoForm==="futuro"?"#fff":"#FDE047", border:`3px solid ${tipoForm==="futuro"?"#fff":"#FDE047"}`,
        ...
        {tab === "retos" && (
          <div style={{ background:"#FDE047", border:"4px solid #000", padding:28, textAlign:"center" }}>
            ...
            <button onClick={() => setReto(RETOS[Math.floor(Math.random() * RETOS.length)])}
              style={{ background:"#000", border:"4px solid #000", color:"#FDE047", padding:"14px 28px",
              ...
              <div style={{ background:"#000", color:"#FDE047", padding:24, marginTop:20, textAlign:"left" }}>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:8 }}>
                  ...
                  <button onClick={() => { setTipoForm("futuro"); setForm(f=>({...f,plan:reto.nombre,highlight:reto.desc,categoria:reto.cat})); setLastRow(null); setTab("nueva"); }}
                    style={{ background:"#FDE047", color:"#000", border:"none", padding:"6px 16px",
